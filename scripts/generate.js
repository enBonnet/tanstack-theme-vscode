 ...
 * @prop {Record<'base'|'ansi'|'brightOther'|'other', string[]>} tanstack - TanStack color variables.
...
const transformSoft = theme => {
    ...
    const brightColors = [...soft.tanstack.ansi, ...soft.tanstack.brightOther];
...
module.exports = async () => {
    const yamlFile = await readFile(
        join(__dirname, '..', 'src', 'tanstack.yml'),
        'utf-8'
    );
